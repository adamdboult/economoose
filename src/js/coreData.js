...
myApp.controller("mainController", [
  ...
  function ($rootScope, $scope, $http, $window) {
    ...
    $http
      ...
      .then(function (response) {
        ...
        for (i = 0; i < Object.keys($scope.filterOptions).length; i++) {
          ...
          if (
            $scope.visibleFilterOptions.indexOf(
              Object.keys($scope.filterOptions)[i],
            ) === -1
          ) {
        ...
        for (var defSearch in defaultSearchArray) {
          $scope.filterChoice[defaultSearchArray[defSearch]] =
            defaultSearchValues[defSearch];
        }
        $scope.filterChange();
      });
    ...
    $http.get("/favs/a").then(function (response) {
      console.log("START");
    ...
    $scope.filterSend = function () {
      ...
      for (var i in $scope.visibleFilterOptions) {
        filterSend[$scope.visibleFilterOptions[i]] =
          $scope.filterChoice[$scope.visibleFilterOptions[i]];
      }
      ...
      $http
        .get("/cpi/" + encodeURIComponent(JSON.stringify(filterSend)))
        .then(function (response) {
      ...
      for (i = 0; i < Object.keys($scope.seriesBrowse).length; i++) {
        ...
        for (j = 0; j < Object.keys($scope.localSeries).length; j++) {
          if ($scope.localSeries[j]._id === $scope.seriesBrowse[i]._id) {
            $scope.seriesBrowse[i].viewToggle =
              $scope.seriesBrowse[i].viewToggle + 1;
          }
        }
      }
    ...
    $scope.removeFilter = function (val) {
      $scope.visibleFilterOptions.splice(
        $scope.visibleFilterOptions.indexOf(val),
        1,
      );
      var i;
      for (i = 0; i < $scope.invisibleFilterOptions.length; i++) {
    ...
    $scope.updateGraph = function (
      ...
      for (i = 0; i < seriesUsed.length; i++) {
        ...
        if (secondSeriesDataExist) {
          ...
          $scope.seriesNameX[i] =
            $scope.localSeries[seriesUsed[i]].filter[xUnits];
        }
        if (firstSeriesDataExist) {
          ...
          $scope.seriesNameY[i] =
            $scope.localSeries[secondSeriesUsed[i]].filter[xUnits];
        }
        ...
        if (jqplotdatay !== "") {
          for (key in jqplotdatay) {
            ...
            if (jqplotdataDot !== "") {
              arrPointDot = parseFloat(
                String(jqplotdataDot[key]).replace(",", ""),
              );
            } else if (dotSeriesUsed[i] === -1) {
      ...
      for (i = 0; i < seriesUsed.length; i++) {
        ...
        if (dotMax > dotMin) {
          $scope.dotRatio[i] =
            (dotUpperRatio - dotLowerRatio) / (dotMax - dotMin);
      ...
      for (i = 0; i < $scope.jq2plotdata.length; i++) {
        ...
        //exp
        var lnymin = 1;
        var lnyOffset = Math.max.apply(null, [
          0,
          lnymin - Math.min.apply(null, yArray),
        ]);
        for (j = 0; j < $scope.jq2plotdata[i].length; j++) {
          lnyTotal += Math.log($scope.jq2plotdata[i][j][1] + lnyOffset);
          lnylnyTotal +=
            Math.log($scope.jq2plotdata[i][j][1] + lnyOffset) *
            Math.log($scope.jq2plotdata[i][j][1] + lnyOffset);
          xlnyTotal +=
            Math.log($scope.jq2plotdata[i][j][1] + lnyOffset) *
            $scope.jq2plotdata[i][j][0];
        }
        ...
        $scope.bHat[i] =
          (xyTotal - (xTotal * yTotal) / nCount) /
          (xxTotal - (xTotal * xTotal) / nCount);
        $scope.intercept[i] =
          $scope.yAverage[i] - $scope.bHat[i] * $scope.xAverage[i];
        ...
        $scope.expbHat[i] =
          (xlnyTotal - (xTotal * lnyTotal) / nCount) /
          (xxTotal - (xTotal * xTotal) / nCount);
        //			$scope.expintercept[i]=$scope.lnyAverage[i]-$scope.expbHat[i]*$scope.xAverage[i];
        $scope.expintercept[i] =
          $scope.lnyAverage[i] - $scope.expbHat[i] * $scope.xAverage[i];
        ...
        for (j = 0; j <= lineFidelity; j++) {
          var xLinePoint =
            Math.min.apply(null, xArray) * ((lineFidelity - j) / lineFidelity) +
            Math.max.apply(null, xArray) * (j / lineFidelity);
          //OLS
          linePoint = [
            xLinePoint,
            $scope.bHat[i] * xLinePoint + $scope.intercept[i],
          ];
          ...
          linePoint = [
            xLinePoint,
            Math.exp($scope.expintercept[i]) *
              Math.exp($scope.expbHat[i] * xLinePoint) -
              lnyOffset,
          ];
    ...
    $scope.drawGraph = function () {
      ...
      var paddingmarginybottom =
        (h - $scope.localSelectionY.length * 20) / 10 +
        $scope.xlabel.length * 15 +
        $scope.localSelectionY.length * 20;
      //scales
      var xMinArray = [];
      var xMaxArray = [];
      var yMinArray = [];
      ...
      for (i = 0; i < $scope.jq2plotdata.length; i++) {
        xMinArray.push(
          d3.min($scope.jq2plotdata[i], function (d) {
            return d[0];
          }),
        );
        xMaxArray.push(
          d3.max($scope.jq2plotdata[i], function (d) {
            return d[0];
          }),
        );
        yMinArray.push(
      ...
      var tickFormatSub = function (d, digitsCount) {
        ...
        return (
          d / Math.pow(10, reduction * 3) + scaleExtension[String(reduction)]
        );
      };
      var xAxis = d3
        .axisBottom(xScale)
      ...
      xGridAppend.selectAll(".grid .tick").style(gridTickStyle);
      ...
      $scope.modelFormula =
        "\\(" + $scope.statsFormula[$scope.lineMatch] + "\\)";
      ...
      var mouseOverFunction = function (d) {
        ...
        div
          ...
          .style(
            "left",
            d3.event.pageX - mainId.getBoundingClientRect().left + "px",
          )
          .style(
            "top",
            d3.event.pageY - mainId.getBoundingClientRect().top - 35 + "px",
          );
      };
      ...
      for (i = 0; i < $scope.jq2plotdata.length; i++) {
        ...
        var legendx = 50;
        var legendy =
          h - paddingmarginybottom + paddingmarginytop + i * 20 + 10;
        //			var legendText="legend"+i;
        var legendSize = 5;

        svg
          .selectAll("legendIcon" + i)
          .data([1])
          .enter()
          .append("circle")
          .attr("cx", legendx)
      ...
      xAxisAppend.selectAll("text").style({ font: "12px sans-serif" });
      var yAxisAppend = svg
        .append("g")
        .attr("class", "axis")
        .attr("transform", "translate(" + paddingmarginxleft + ",0)")
      ...
      yAxisAppend.selectAll("text").style({ font: "12px sans-serif" });
      ...
      for (i = 0; i < $scope.xlabel.length; i++) {
        svg
          ...
          .attr(
            ...
            10 +
              h -
              paddingmarginybottom +
              paddingmarginytop +
              13 * (i - $scope.xlabel.length),
          )
          .text($scope.xlabel[i])
    ...
    $scope.localGraph = function () {
      ...
      for (i = 0; i < $scope.localSelectionSize.length; i++) {
        if ($scope.localIndex.indexOf($scope.localSelectionSize[i]) > -1) {
          seriesMatchDot.push(
            $scope.localIndex.indexOf($scope.localSelectionSize[i]),
          );
        } else {
    ...
    $scope.JSONToCSVConvertor = function () {
      ...
      for (i = 0; i < arrData.length; i++) {
        if (arrData[i].data !== -1) {
          row +=
            '"' + arrData[i].filter[xUnits] + '","' + arrData[i].label + '",,';
      ...
      for (i = 0; i < lengthCheck; i++) {
        ...
        for (j = 0; j < arrData.length; j++) {
          if (arrData[j].data !== -1) {
            row +=
              '"' +
              Object.keys(arrData[j].data)[i] +
              '","' +
              arrData[j].data[Object.keys(arrData[j].data)[i]] +
              '",,';
          }
        }
        CSV += row + "\r\n";
      ...
      for (i = 0; i < filterLength; i++) {
        ...
        for (j = 0; j < arrData.length; j++) {
          if (arrData[j].data !== -1) {
            row +=
              '"' +
              Object.keys(arrData[j].filter)[i] +
              '","' +
              arrData[j].filter[Object.keys(arrData[j].filter)[i]] +
              '",,';
          }
        }
        CSV += row + "\r\n";
      ...
      for (i = 0; i < noteLength; i++) {
        ...
        for (j = 0; j < arrData.length; j++) {
          if (arrData[j].data !== -1) {
            row +=
              '"' +
              Object.keys(arrData[j].note)[i] +
              '","' +
              arrData[j].note[Object.keys(arrData[j].note)[i]] +
              '",,';
          }
        }
        CSV += row + "\r\n";
